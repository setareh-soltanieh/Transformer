...
def greedy_decode(model, encoder_input, encoder_mask, tokenizer_src, tokenizer_tgt, max_len, device):
    ...
    # (Batch, Token of the decoder input)
    decoder_input = torch.empty(1, 1).fill_(sos_index).type_as(encoder_input).to(device)
    # we will ask the decoder to predict the next token until we either get to the EOS token ot the max_len
    while True:
        ...
        # get the next token, we only use the output of the last token
        prob = model.project(out[:, -1])
        # Select the token with maximum probability
        _, next_word = torch.max(prob, 1)
...
def run_validation(model, validation_ds, tokenizer_src, tokenizer_tgt, max_len, device, print_msg, global_state, writer, num_examples=2):
    ...
    # Size of the control window (just use a different value)
    console_width = 80
    ...
    with torch.no_grad():
        for batch in validation_ds:
            ...
            model_out_text = tokenizer_tgt.decode(model_output.detach().cpu().tolist())
...
def get_or_build_tokenizer(config, ds, lang):
    ...
    if not Path.exists(tokenizer_path):
        ...
        # if we want to add a word to the vocabulary it has to ne repeated at least two times
...
def get_ds(config):
    ...
    # keep 90% for training and 10% for testing
...
def train_model(config):
    ...
    # make sure the weights folder is created
    ...
    # visualize the charts with tensorboard
    ...
    # here we use label_smoothing to make the model more robust and prevent it from overfitting, improve its generalization and make the training more stable
